/**
 * Seed 2 apps via API
 * Run: node scripts/seed-apps.mjs
 */
...
async function seedApps() {
  console.log('🌱 Seeding apps via API...\n');

  for (const app of apps) {
    try {
      const response = await fetch('http://localhost:3456/api/admin/apps', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error(`❌ Failed to create ${app.slug}:`, error);
      }
    } catch (error) {
      console.error(`❌ Error with ${app.slug}:`, error.message);
    }
  }
